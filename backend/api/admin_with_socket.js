...
// GET /api/admin/students - Get all students with filters (1340 students support)
...
// PUT /api/admin/student/:regdNo - Update student (with socket communication)
router.put('/student/:regdNo', async (req, res) => {
  try {
    ...
    if (result.success) {
      // Emit real-time update to student portal
      if (io) {
        const studentData = {
        ...
        io.to(`student:${regdNo.toUpperCase()}`).emit('student_profile_updated', studentData);
        
        // Send to all admin rooms for synchronization
        io.emit('admin_student_updated', studentData);

        console.log(`[Socket] Student profile update sent to student:${regdNo}`);
      }
...
// POST /api/admin/bulk-marks - Bulk marks entry (with socket communication)
  ...
  try {
    ...
    for (const markEntry of marks) {
      try {
        ...
        // Collect updated student data for socket emission
        updatedStudents.push({
          regdNo: markEntry.regd_no.toUpperCase(),
          subject,
          testType,
          marks: markEntry.marks,
          maxMarks: markEntry.max_marks,
    ...
    // Emit real-time updates to all affected students
    if (io && updatedStudents.length > 0) {
      updatedStudents.forEach(studentData => {
        // Send to specific student room
        io.to(`student:${studentData.regdNo}`).emit('marks_updated', studentData);
      });

      // Send bulk update notification to admins
      io.emit('bulk_marks_completed', {
        section,
        subject,
        testType,
        savedCount,
      ...
      console.log(`[Socket] Bulk marks update sent to ${updatedStudents.length} students`);
    }
...
// POST /api/admin/student/:regdNo/attendance - Update attendance (with socket communication)
  ...
  try {
    ...
    if (result.success) {
      ...
      if (io) {
        ...
        // Send to specific student room
        io.to(`student:${regdNo.toUpperCase()}`).emit('attendance_updated', attendanceData);
        
        // Send to all admin rooms for synchronization
        io.emit('admin_attendance_updated', attendanceData);

        console.log(`[Socket] Attendance update sent to student:${regdNo}`);
      }
...
// POST /api/admin/notice - Post notice (with socket communication)
router.post('/notice', async (req, res) => {
  try {
    ...
    if (result.success) {
      // Emit real-time notice to students
      if (io) {
        const noticeEmitData = {
          ...noticeData,
          id: result.notice.id,
          timestamp: new Date().toISOString()
        };
        ...
        if (targetRegdNo) {
          ...
          io.to(`student:${targetRegdNo.toUpperCase()}`).emit('new_notice', noticeEmitData);
        } else {
          // Send to all students
          io.emit('new_notice', noticeEmitData);
        }

        // Send to all admins for synchronization
        io.emit('admin_notice_posted', noticeEmitData);

        console.log(`[Socket] Notice sent to ${targetRegdNo ? 'student:' + targetRegdNo : 'all students'}`);
      }
...
// POST /api/admin/auto-attendance - Generate attendance based on CGPA (with socket communication)
  ...
  try {
    ...
    for (const student of studentsResult.rows) {
      ...
      try {
        ...
        // Collect updated student data for socket emission
        updatedStudents.push({
          regdNo: student.regd_no.toUpperCase(),
          attendance: parseFloat(attendance.toFixed(2)),
          cgpa,
          timestamp: new Date().toISOString(),
          updatedBy: 'auto-generation'
        });
      } catch (err) {
    ...
    if (io && updatedStudents.length > 0) {
      updatedStudents.forEach(studentData => {
        // Send to specific student room
        io.to(`student:${studentData.regdNo}`).emit('attendance_updated', studentData);
      });

      // Send bulk update notification to admins
      io.emit('auto_attendance_completed', {
        branch,
        semester,
        updatedCount,
      ...
      console.log(`[Socket] Auto-attendance update sent to ${updatedStudents.length} students`);
    }
